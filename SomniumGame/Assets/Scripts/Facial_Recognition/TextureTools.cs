...
namespace TFClassify
{
    public class TextureTools
    {
        ...
        public static Texture2D RotateTexture(Texture2D originTexture, int angle) {
            var result = RotateImageMatrix(originTexture.GetPixels32(), originTexture.width, originTexture.height, angle);
        ...
        public static Color32[] RotateImageMatrix(Color32[] matrix, int width, int height, int angle)
        {
            ...
            Color32[] pix3 = rotateSquare(matrix, width, height, (Math.PI/180*(double)angle));
            UnityEngine.Debug.Log($"pix3: {pix3[0]}");
            ...
            for (j = 0; j < height; j++){
                for (i = 0; i < width; i++)
                {
                    ...
                    if ((x > -1) && (x < width) && (y > -1) && (y < height)) {
                        //Array.Copy(arr, (y * width + x), arr2, (j * width + i), 1);
                        arr2[j * width + i] = arr[y * width + x];
                        UnityEngine.Debug.Log(arr[y * width + x]);
                        UnityEngine.Debug.Log(arr2[y * width + x]);
